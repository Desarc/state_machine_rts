...
function STMTrafficLightController:fire()
	...
	while(true) do
		...
		elseif current_state == S1 then
			if event:type() == self.events.YELLOW_TIMER_EXPIRED and event:timer():id() == self:id()..T1 then
		...
		elseif current_state == S2 then
			if event:type() == self.events.PEDESTRIANS_GO and event:timer():id() == self:id()..T2 then
		...
		elseif current_state == S3 then
			if event:type() == self.events.PEDESTRIAN_TIMER_EXPIRED and event:timer():id() == self:id()..T3 then
				print("Pedestrian light set to red.")
		...
		elseif current_state == S4 then
			if event:type() == self.events.CARS_GO and event:timer():id() == self:id()..T4 then
				print("Car light set to yellow.")
		...
		elseif current_state == S5 then
			if event:type() == self.events.YELLOW_TIMER_EXPIRED and event:timer():id() == self:id()..T5 then
				print("Car light set to green.")
				self:set_state(S0)
				coroutine.yield(StateMachine.EXECUTE_TRANSITION)
